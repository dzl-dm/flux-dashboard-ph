...
def process_data(input_path_main, input_path_patients, input_path_ATC_mapping, Visit_cutoff_days = 7, Baseline_tolerance_days = 90):
    '''
...
# #---------------------------------------------------------------------------------------------
    ...
    df_multi = df_multi.sort_values("start").drop_duplicates(subset=["patient_id"], keep="first")
    ...
    df_params = df_patients.merge(df_params, left_on="patient_id", right_on="patient_id", how="left")
    ...
    df_params["deceaseddate"] = pd.to_datetime(df_params["deceaseddate"], format=time_format)

    # calculate "age at diagnosis"
    df_params["age_at_diagnosis"] = df_params["start"].dt.year - df_params["birthdate"].dt.year
    ...
    df_med = df_med.sort_values("start").drop_duplicates(subset=["patient_id", "code"], keep="first").reset_index()

    df_med = df_med.merge(df_atc_mapping[["med_name", "ATC_code"]], right_on="ATC_code", left_on="code", how="left")

    df_med.drop(columns=["end","type","value","unit","ATC_code","index"], inplace=True)
    ...
    df_filtered = df_main[df_main["code"].isin(param_lab + param_rhc + param_pft + param_echo + param_rhc_or)].copy()

    # convert datatype to datetime
    df_filtered["start"] = pd.to_datetime(df_filtered["start"], format=time_format).dt.date
    ...
    def assign_visits(group):
        ...
        for _, row in group.iterrows():
            ...
            if prev_date is None:
                new_visit = True
            # 2. the previous visit is more than *Visit_cutoff_days* in the past
            elif (row["start"] - prev_date).days > Visit_cutoff_days:
                new_visit = True
    ...
    .apply(assign_visits, include_groups=False)
    )
    ...
    df_wide = (
        df_filtered
        .pivot_table(index=["patient_id", "visit_id"], columns="code", values="value", aggfunc="count")
        .reset_index()
    )

    df_wide.columns.name = None

    df_wide = df_wide.reindex(columns=df_wide.columns.union(param_lab + param_rhc + param_pft + param_echo + param_rhc_or))
    ...
    df_params['diagnosis_date'] = pd.to_datetime(df_params['diagnosis_date'], format=time_format)
    ...
    df_wide = df_wide.merge(df_params[["patient_id", "diagnosis_date"]], on="patient_id", how="left")
    ...
    df_wide['baseline_flag'] = (
        (df_wide['diagnosis_date'] >= df_wide['visit_start'] - pd.Timedelta(days=Baseline_tolerance_days)) &
        (df_wide['diagnosis_date'] <= df_wide['visit_end'] + pd.Timedelta(days=Baseline_tolerance_days))   
    )

    # flag the visits for each of the measurement methods
    df_wide["visitw_lab"] = df_wide[param_lab].notna().all(axis=1).astype(int)
    df_wide["visitw_rhc"] = (df_wide[param_rhc].notna().all(axis=1).astype(int) & df_wide[param_rhc_or].notna().any(axis=1).astype(int)
    )
    ...
    # identify columns
    meta_cols = ["patient_id", "visit_id", "visit_start", "visit_end", "diagnosis_date", "baseline_flag", "visit_gap"]
    ...
    df_base_counts = (
        ...
        .size()
        .reset_index(name="baseline_visit_count")
    )
    ...
    df_params = (df_params
                ...
                on="patient_id",
                how="left"
                ).merge(df_non_base_counts.rename(columns={"patient_id": "patient_id"}),
                on="patient_id",
                how="left"
                )
    )
    ...
    # assign new visit_id
    df_base_agg["visit_id"] = (
        df_base_agg["patient_id"].astype(str) + "_BASELINE"
    )
    ...
    df_base = df_wide.loc[mask, ["visitw_lab", "visitw_rhc", "visitw_pft", "visitw_echo", "patient_id"]]
    df_params = df_params.merge(df_base, right_on = "patient_id" , left_on= "patient_id", how="left" ).rename(columns=
        {
    ...
    (df_wide["visit_start"].dt.year - df_wide["diagnosis_date"].dt.year) * 12 +
    (df_wide["visit_start"].dt.month - df_wide["diagnosis_date"].dt.month)
    )

    #convert datetime to date
    df_params["birthdate"] = pd.to_datetime(df_params["birthdate"], format=time_format).dt.date
    # df_params["start"] = pd.to_datetime(df_params["start"], format=time_format).dt.date
    df_params["deceaseddate"] = pd.to_datetime(df_params["deceaseddate"], format=time_format).dt.date
    df_params['diagnosis_date'] = pd.to_datetime(df_params['diagnosis_date'], format=time_format).dt.date
    df_wide['visit_start'] = pd.to_datetime(df_wide['visit_start'], format=time_format).dt.date
    df_wide['visit_end'] = pd.to_datetime(df_wide['visit_end'], format=time_format).dt.date
    df_wide['diagnosis_date'] = pd.to_datetime(df_wide['diagnosis_date'], format=time_format).dt.date

    # identify patients with a baseline visit 
    patients_with_baseline = df_wide.loc[
    df_wide["visit_id"].str.endswith("BASELINE"),
    "patient_id"
    ].unique()
    # compute the number of RHC per patient
    df_n_rhc = (df_wide[df_wide["patient_id"]
                    .isin(patients_with_baseline)]
                    .groupby("patient_id")["visitw_rhc"]
                    .sum().reset_index(name="fcath_count")) 
    
    # identify the max "diagnosis_gap" per patient
    df_max_gap = df_wide.groupby("patient_id")["diagnosis_gap_months"].max().reset_index(name="max_diagnosis_gap_months")

    df_lat_visit = df_wide.groupby("patient_id")["visit_end"].max().reset_index(name="latest_visit_date")
    ...
    df_wide.to_parquet("df_wide_05_19.gzip", compression = "gzip")
    df_params.to_parquet("df_params_05_19.gzip", compression = "gzip")
    df_med.to_parquet("df_med_05_19.gzip", compression = "gzip")
